...
from qcodes_addons.BaselAWG5204 import BaselAWG5204  # Modified AWG

# Import helper functions
from qcodes_addons.Datahelp import (
    dataset_to_numpy,
    datasetID_to_numpy,
    define_detuning,
    plot2D_by_ID,
...
from qcodes_addons.E8257D import Agilent_E8527D
from qcodes_addons.Lakeshore_331 import Model_331
...
from qcodes_addons.Parameterhelp import (
    AxisParameter,
...
from qcodes_addons.SGS100A import RohdeSchwarz_SGS100A
...
def initialise_experiment(gate_set='A'):
    db_name = "GeSiNW_Qubit_VTI01_Jonas_10.db"  # Database name
    sample_name = "Butch"  # Sample name
    exp_name = f'gate set {gate_set}'  # Experiment name
    ...
    print('Trying to connect to DAC...')
    #BasPI DAC 8-chanel
    dac = SP927('LNHR_dac', 'ASRL2::INSTR')
    station.add_component(dac)

    print('Trying to connect to MFLI...')
    #MFLI Zurich Instruments
    mfli = ziqc.MFLI("dev5626",host="127.0.0.1",name = "mfli",  interface="USB",)
    station.add_component(mfli)

    print('Trying to connect to DAQ...')
    daq_chs = [0,1,2,3,4]
    ...
    print('Trying to connect to IPS...')
    ...
    print('Trying to connect to AWG...')
    # 4-channel Tektronics AWG5204
    awg = BaselAWG5204('awg', 'TCPIP0::192.168.10.3::INSTR') #driver adapted from 5208 version, setting configs in AWG70000A the same as for 5204 as for 5208
    ...
    if gate_set == 'A':
        ...
        channel1 = GateParameter(dac.ch1.volt,
                           name="ch1",
                           unit="V",
                           value_range=(-1, 2))

        VL = GateParameter(dac.ch2.volt,
                           name="V_L",
                           unit="V",
                           value_range=(-1, 2))

        VLP = CompensatedGateParameter(dac.ch3.volt,
                            name="V_LP",
                            value_range=(-1, 2),
                            pp=pp)

        VM = GateParameter(dac.ch4.volt,
                           name="V_M",
                           unit="V",
                           value_range=(-1, 2))
        ...
        VRP = CompensatedGateParameter(dac.ch5.volt,
                                       name="V_RP",
                                       value_range=(-1, 2),
                                       pp=pp)

        VR = GateParameter(dac.ch6.volt,
                           name="V_R",
                           unit="V",
                           value_range=(-1, 2.51))
    elif gate_set == 'C':
        channel1 = GateParameter(dac.ch1.volt,
                           ...
                           unit="V",
                           value_range=(-1, 2))
        channel2 = GateParameter(dac.ch2.volt,
                                 name="ch2",
                                 unit="V",
                                 value_range=(-1, 2))

        VL = GateParameter(dac.ch3.volt,
                           name="V_L",
                           unit="V",
                           value_range=(-1, 2))

        VLP = CompensatedGateParameter(dac.ch4.volt,
                            name="V_LP",
                            value_range=(-1, 2),
                            pp=pp)

        VM = GateParameter(dac.ch5.volt,
                           name="V_M",
                           unit="V",
                           value_range=(-1, 2))
        ...
        VRP = GateParameter(dac.ch6.volt,
                           name="V_RP",
                            unit="V",
                            value_range=(-1, 2))

        VR = GateParameter(dac.ch7.volt,
                           name="V_R",
                           unit="V",
                           value_range=(-1, 3.01))
    else:
    ...
    components_to_add = [ISD, VLP, VRP, VL, VM, VR, LIX, LIY, LIR, LIPhi, ips.field_setpoint, VSD, VS_freq, LITC]
    ...
    return station, ips,awg, pp, DAQ, mfli, VS, VS_freq, VS_phase, VS_pwr, VS_pulse_on, VS_pulse_off, VS_IQ_on, VS_IQ_off, VS_status, LIXY, LIXYRPhi, LIX, LIY, LIR, LIPhi, LIPhaseAdjust, LIfreq, LITC, ISD, DAQ, VS, VM, VL, VLP, VR, VRP, VSD
...
if __name__=='__main__':
    station, ips, awg, pp, DAQ, mfli, VS, VS_freq, VS_phase, VS_pwr, VS_pulse_on, VS_pulse_off, VS_IQ_on, VS_IQ_off, VS_status, LIXY, LIXYRPhi, LIX, LIY, LIR, LIPhi, LIPhaseAdjust, LIfreq, LITC, ISD, DAQ, VS, VM, VL, VLP, VR, VRP, VSD = initialise_experiment()