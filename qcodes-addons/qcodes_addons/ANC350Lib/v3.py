...
class ANC350v3Lib:
    ...
    def discover(self, search_usb: bool = True, search_tcp: bool = False) -> int:
        '''
        The function searches for connected ANC350RES devices on USB and LAN 
        and initializes internal data structures per device. Devices that are 
        in use by another application or PC are not found. The function must be 
        called before connecting to a device and must not be called as long as 
    ...
    def get_device_info(self, dev_no: int = 0) -> Tuple[ANC350LibDeviceType, int, str, str, bool]:
        ...
        called before "discover", but the devices don't have to be connected 
        with "connect". All Pointers to output parameters may be zero to ignore 
        the respective value.

        Args:
            dev_no: Sequence number of the device. Must be smaller than the 
                    return value from the last "discover" call (default: 0).
        ...
        Returns:
            A tuple containing:
                ...
                3. address: The device's interface address if applicable. 
                Returns the IP address in dotted-decimal notation or the 
                string "USB", respectively
