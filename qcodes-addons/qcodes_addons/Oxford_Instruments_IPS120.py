...
# Comment Rafael Eggli, 15.12.2021:
# Make sure to include correct Settings of the IPS (such as sweep rate, max field etc.) !!!!
...
class OxfordInstruments_IPS120(VisaInstrument):
    ...
    def __init__(self, name, address, use_gpib=False, number=2, baud_rate=9600, data_bits = 8, **kwargs):
        ...
        maximal_field = 8   #highest safe field
        minimal_field = -8  #lowest safe field
        maximal_sweep_rate = 0.15 #T/min
        ...
        self.add_parameter('field_setpoint',
                           ...
                           vals=vals.Numbers(minimal_field, maximal_field))
        self.add_parameter('field_setpoint_wait',
                           unit='T',
                           get_cmd=self._get_field_setpoint_wait,
                           set_cmd=self._set_field_setpoint_wait,
                           vals=vals.Numbers(minimal_field, maximal_field))
        self.add_parameter('sweeprate_field',
                           unit='T/min',
                           get_cmd=self._get_sweeprate_field,
                           set_cmd=self._set_sweeprate_field,
                           vals=vals.Numbers(0, maximal_sweep_rate))
        ...
        # A to B conversion
        ABconversion = 45.240 / 4 # Ampere per Tesla
    ...
    def _get_field_setpoint_wait(self):
        """
    ...
    def _set_field_setpoint_wait(self, field):
        """
    ...
    def initialise_magnet(self):
        """
        Run when you start using the magnet once
        
        
        """
        self.remote()
        self._set_sweeprate_field(0.1)
        self.to_setpoint()
        self.examine()
   

    def deinitialise_magnet(self):
        """
        Run before you stop using the magnet
        ...
        self.heater_on()
        sleep(40)
        self._set_field_setpoint(0.000)
        sleep(10)
        self.heater_off()
        self.to_zero()
        self.examine()
